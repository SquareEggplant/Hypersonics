...
%% Function Call
a = run_old1d_solver('tungsten.txt','Trajectory_test_aero_grids_dense.csv',0.9,0.3,0.003,300,"2D","nose");
% b = run_old1d_solver('tungsten.txt','Trajectory_test_aero_grids_dense.csv',0.9,0.01,0.002,10,0,0);
% c = run_old1d_solver('tungsten.txt','Trajectory_test_aero_grids_dense.csv',0.9,0.01,0.003,10,0,0);
% d = run_old1d_solver('tungsten.txt','Trajectory_test_aero_grids_dense.csv',0.9,0.01,0.004,10,0,0);
% e = run_old1d_solver('tungsten.txt','Trajectory_test_aero_grids_dense.csv',0.9,0.01,0.005,10,0,0);
% f = run_old1d_solver('tungsten.txt','Trajectory_test_aero_grids_dense.csv',0.9,0.01,0.006,10,0,0);
% g = run_old1d_solver('tungsten.txt','Trajectory_test_aero_grids_dense.csv',0.9,0.01,0.007,10,0,0);
% h = run_old1d_solver('tungsten.txt','Trajectory_test_aero_grids_dense.csv',0.9,0.01,0.008,10,0,0);
% i = run_old1d_solver('tungsten.txt','Trajectory_test_aero_grids_dense.csv',0.9,0.01,0.009,10,0,0);
% j = run_old1d_solver('tungsten.txt','Trajectory_test_aero_grids_dense.csv',0.9,0.01,0.01,10,0,0);
%ylim([2500 3500])
%legend('1 mm','2 mm','3 mm','4 mm','5 mm','6 mm','7 mm','8 mm','9 mm','10 mm')
%%
clear;
...
%%
function [T_inshistory] = run_insulation_solver(inputfile,altitudefile,layer,Nx,T_history,mode,dt)
    warning('off','all');
    ...
    dxtot = 0;
    dx = zeros(size(layer,1));
    for i = 1:size(layer,1)
        dx(i) = layer(i)/(Nx-1);    % Spatial Steps
        dxtot = dxtot + dx;
    end
    time = altitude(end,1); % Time [s]
    Nt = ceil(time/dt); % # of Time Partitions
    T = zeros(Nx,1) + 288.16;      % Initial Temperature Profile (ITP)
    T_new = T;                    % Updated Temperature Profile (Initially the same as T)
    ...
    for p = 1:Nt-1
        fprintf('Progress: %g%%\n' , round(p/Nt*100,2))
        fprintf('Time: %g\n', p*dt)
        %T_new(2) = T(2)+alpha*dt/dx^2*(T_history(1,p)-T_history(1,p+1));
        for i = 2:Nx-1
            % Update the Internal Nodes
            T_new(i) = (alpha*dt/dx^2)*(T(i+1)+T(i-1)) + (1-2*(alpha*dt/dx^2))*T(i);
            for j = 2:size(dx,1)
                if i>dx(j)
                    data = accessMat(inputfile{j});
                    alpha = data.ThermalConductivity_W_m__C_/(data.Density_kg_m_3_*data.SpecificHeatCapacity_J_kg__C_); % Thermal diffusivity [m^2/s]
                end
            end
        end
        % Update the Edge Node
        %T_new(2) = alpha*dt/dx^2*(T(3)-T(2)) + T(2);
        T_new(1) = T(1) + (T_new(2)-T(2));
        % Update conductive end BC
        if mode == 1
        T_new(Nx) = T(Nx) + (alpha*dt/dx^2)*(T(Nx-1)-T(Nx));
        elseif mode == 0
        T_new(Nx) = 288.16;
        end
        % Update for Next Time Step
    ...
    % for i = 1:Nt-1
    %     T_inshistory(1,i) = T_history(end,i);
    % end
    [x,t] = meshgrid(linspace(0,layer(1)+layer(2)+layer(3),Nx),linspace(0,time,Nt));
	...
	shading interp;
    % figure;
    % plot(linspace(0,time,Nt),T_history(1,:));
end
%% 1-Dimensional Implicit Heat Analysis with conduction, convection, and radiation
function [T_history] = run_old1d_solver(inputfile,altitudefile,eps,L,radius,Nx,mode,hm)
warning('off','all');
% Access Material Properties
%inputfile = 'carbonCeramic.txt';
data = accessMat(inputfile);
% Accesses specific heat data based on temperature
cpfit = air('N2.txt');
alpha = data.ThermalConductivity_W_m__C_/(data.Density_kg_m_3_*data.SpecificHeatCapacity_J_kg__C_); % Thermal diffusivity [m^2/s]
[altitude,rhofit,tempfit,localM] = altgen(altitudefile);
%eps = 0.5;        % Emissivity of Material
sig = 5.67e-8;    % Stefan-Boltzmann Constant
% Time and Spatial Step Initialization
...
elseif mode == "2D"
    ...
    xlabel('Position [m]');
	ylabel('Time [s]');
    title('Nose Cone Thermal Simulation');
end
figure;
tiledlayout(2,3);
