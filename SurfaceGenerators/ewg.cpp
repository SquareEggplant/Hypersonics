//EXTRUDED WAVERIDER GENERATOR
//ALL ISSUES ARE NO LONGER FAULT OF ERIC JO
//CHRISTOPH BREHM AND JOHAN LARSSON ALSO SHARE RESPONSIBILITY
...
void write_vtk(vec** bot_arr, vec** top_arr, vec** RLE, int roundpoints, vec** aft_edge, int aft_rows,
    int num_points, int* num_samples, int seednum, const std::string& filename) {
    ...
    vtk_file << "Might put some dimension limits here in the future (Length, Planar Area, X limit)\n";
    vtk_file << "ASCII\n";
    ...
    std::vector<int>* ae_points = new std::vector<int>[aft_rows*2];
    ...
    for(int i=0; i<2*aft_rows;i++){
        for(int j=0; j<roundpoints; j++){
            vtk_file << aft_edge[i][j].x << " " << aft_edge[i][j].y  << " " << aft_edge[i][j].z  << "\n";
            ae_points[i].push_back(ind);
            ind++;
        }
    }
    ...
    for(int i=0; i<roundpoints-1; i++){
        temp.comp = 2;
        temp.x = bottom_points[0][aft_rows]; temp.y = ae_points[aft_rows-1][i+1]; temp.z = ae_points[aft_rows-1][i];
        triangles.push_back(temp); 
        temp.x = bottom_points[seednum-1][aft_rows]; temp.y = ae_points[2*aft_rows-1][i]; temp.z = ae_points[2*aft_rows-1][i+1];
        triangles.push_back(temp); 
    }

    temp.x = bottom_points[0][aft_rows]; temp.y = ae_points[aft_rows-1][0]; temp.z = top_points[0][aft_rows];
    triangles.push_back(temp);
    temp.x = bottom_points[seednum-1][aft_rows]; temp.y = top_points[seednum-1][aft_rows]; temp.z = ae_points[2*aft_rows-1][0];
    triangles.push_back(temp);

    //Aft Edges (Negative-x edge)
    for(int i=0; i<aft_rows-1; i++){
        temp.comp = 3;
        for(int j=0; j<roundpoints-1;j++){
            temp.x = ae_points[i][j]; temp.y = ae_points[i+1][j]; temp.z = ae_points[i+1][j+1];
            triangles.push_back(temp);
            temp.x = ae_points[i][j]; temp.y = ae_points[i+1][j+1]; temp.z = ae_points[i][j+1];
            triangles.push_back(temp);
        }
        temp.x = ae_points[i][roundpoints-1]; temp.y = ae_points[i+1][roundpoints-1]; temp.z = bottom_points[0][i+2];
        triangles.push_back(temp);
        temp.x = ae_points[i][roundpoints-1]; temp.y = bottom_points[0][i+2]; temp.z = bottom_points[0][i+1]; 
        triangles.push_back(temp);
        temp.x = top_points[0][i+1]; temp.y = top_points[0][i+2]; temp.z = ae_points[i+1][0];
        triangles.push_back(temp);
        temp.x = top_points[0][i+1]; temp.y = ae_points[i+1][0]; temp.z = ae_points[i][0]; 
        triangles.push_back(temp);
    }
    for(int j=0; j<roundpoints-1;j++){
        temp.x = le_points[0][j]; temp.y = ae_points[0][j]; temp.z = ae_points[0][j+1];
        triangles.push_back(temp);
        temp.x = le_points[0][j]; temp.y = ae_points[0][j+1]; temp.z = le_points[0][j+1];
        triangles.push_back(temp);
    }
    temp.x = le_points[0][roundpoints-1]; temp.y = ae_points[0][roundpoints-1]; temp.z = bottom_points[0][1];
    triangles.push_back(temp);
    temp.x = le_points[0][roundpoints-1]; temp.y = bottom_points[0][1]; temp.z = bottom_points[0][0]; 
    triangles.push_back(temp);
    temp.x = top_points[0][0]; temp.y = top_points[0][1]; temp.z = ae_points[0][0];
    triangles.push_back(temp);
    temp.x = top_points[0][0]; temp.y = ae_points[0][0]; temp.z = le_points[0][0]; 
    triangles.push_back(temp);

    //Positive-x edge
    for(int i=0; i<aft_rows-1; i++){

        int index = aft_rows+i;
        temp.comp = 3;
        for(int j=0; j<roundpoints-1;j++){
            temp.x = ae_points[index][j]; temp.y = ae_points[index+1][j+1]; temp.z = ae_points[index+1][j]; 
            triangles.push_back(temp);
            temp.x = ae_points[index][j]; temp.y = ae_points[index][j+1]; temp.z = ae_points[index+1][j+1]; 
            triangles.push_back(temp);
        }
        temp.x = ae_points[index][roundpoints-1]; temp.y = bottom_points[seednum-1][i+2]; temp.z = ae_points[index+1][roundpoints-1];
        triangles.push_back(temp);
        temp.x = ae_points[index][roundpoints-1]; temp.y = bottom_points[seednum-1][i+1]; temp.z = bottom_points[seednum-1][i+2];
        triangles.push_back(temp);
        temp.x = top_points[seednum-1][i+1]; temp.y = ae_points[index+1][0]; temp.z = top_points[seednum-1][i+2];
        triangles.push_back(temp);
        temp.x = top_points[seednum-1][i+1]; temp.y = ae_points[index][0]; temp.z = ae_points[index+1][0];
        triangles.push_back(temp);
    }
    for(int j=0; j<roundpoints-1;j++){
        temp.x = le_points[seednum-1][j]; temp.y = ae_points[aft_rows][j+1]; temp.z = ae_points[aft_rows][j];
        triangles.push_back(temp);
        temp.x = le_points[seednum-1][j]; temp.y = le_points[seednum-1][j+1]; temp.z = ae_points[aft_rows][j+1];
        triangles.push_back(temp);
    }
    temp.x = le_points[seednum-1][roundpoints-1]; temp.y = bottom_points[seednum-1][1]; temp.z = ae_points[aft_rows][roundpoints-1];
    triangles.push_back(temp);
    temp.x = le_points[seednum-1][roundpoints-1]; temp.y = bottom_points[seednum-1][0]; temp.z = bottom_points[seednum-1][1]; 
    triangles.push_back(temp);
    temp.x = top_points[seednum-1][0]; temp.y = ae_points[aft_rows][0]; temp.z = top_points[seednum-1][1];
    triangles.push_back(temp);
    temp.x = top_points[seednum-1][0]; temp.y = le_points[seednum-1][0];  temp.z = ae_points[aft_rows][0];
    triangles.push_back(temp);
...
int main(int argc, char* argv[]){
    ...
    double z_center = coneslope*10*rpl;
    ...
    double aft_length = 0.2; 
    double aft_spacing = 0.05;
    int aft_rows = aft_length/aft_spacing;

    vec** top_points = new vec*[seednum];
    vec** bot_points = new vec*[seednum];
    int* extruded_samplenum = new int[seednum];
    for(int i=0; i<seednum; i++) {
        extruded_samplenum[i] = samplenum[i] + aft_rows;
        top_points[i] = new vec[extruded_samplenum[i]];
        bot_points[i] = new vec[extruded_samplenum[i]];
    }
    ...
    //Bottom Surface
    for(int i=0; i<seednum; i++){
        row_samples = samplenum[i];
        
        for(int j=0; j<row_samples; j++){
        ...
        final_point = bot_points[i][row_samples-1];
        for(int j=0; j<aft_rows; j++){
            bot_points[i][row_samples+j] = final_point;
            bot_points[i][row_samples+j].z = final_point.z+(j+1)*aft_spacing;
        }
    }
    
    for(int i=seedsgen; i<seednum; i++){
        int mirror = seednum-1-i;
        for(int j=0; j<extruded_samplenum[mirror]; j++){
    ...
    for(int i=0; i<seednum; i++){
        ...
        for(int j=0; j<row_samples; j++){
            ...
            if(row_samples == 1) top_points[i][j].z = zstart-z_center;
            else top_points[i][j].z = j*(zmax-zstart)/(row_samples-1)+zstart-z_center;
        }

        final_point = top_points[i][row_samples-1];

        for(int j=0; j<aft_rows; j++){
            top_points[i][row_samples+j] = final_point;
            top_points[i][row_samples+j].z = final_point.z+(j+1)*aft_spacing;
        }
    }

    //Rounded leading edge hack
    vec** roundededge = new vec*[seednum];
    vec** aft_edge = new vec*[aft_rows*2];
    for(int i = 0; i<seednum; i++) roundededge[i] = new vec[roundpoints];
    for(int i = 0; i<aft_rows*2; i++) aft_edge[i] = new vec[roundpoints];
    double tol = 1e-5; //Tolerance for how close to (0,1) our inward vector is

    //Endcases
    //Slope of back
    vec temp; temp.z=0; //I gotta stop calling these variables temp
    ...
    for(int i=0; i<aft_rows; i++){
        for(int j=0; j<roundpoints; j++) {
            aft_edge[i][j]=roundededge[0][j];
            aft_edge[i][j].z = roundededge[0][j].z+(i+1)*aft_spacing;           
        }
    }
    for(int i=aft_rows; i<aft_rows*2; i++){
        for(int j=0; j<roundpoints; j++) {
            aft_edge[i][j]=roundededge[seednum-1][j];
            aft_edge[i][j].z = roundededge[seednum-1][j].z+(i-aft_rows+1)*aft_spacing;           
        }
    }
    ...
    int total_points = (seednum+2*aft_rows)*roundpoints;
    for(int i=0; i<seednum; i++) total_points += 2*extruded_samplenum[i];

    write_vtk(bot_points, top_points, roundededge, roundpoints, aft_edge, aft_rows, total_points, extruded_samplenum, seednum, filename);
    return 0;
}